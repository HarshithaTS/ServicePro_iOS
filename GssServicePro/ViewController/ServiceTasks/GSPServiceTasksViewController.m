...
@interface GSPServiceTasksViewController ()<CustomPickerDelegate, UISearchBarDelegate,UIActionSheetDelegate>
{
    ...
    BOOL isOnlyRefresh, isSearching, isSortingAscending;
    int sortedButtonTag;
...
*/
    GSPAppDelegate *appDelegateObj = (GSPAppDelegate *) [[UIApplication sharedApplication] delegate];
    if (self.screenType == serviceTaskOverView  && !appDelegateObj.saveChangesFlag && !appDelegateObj.didBecomeActive)
    {
        [self getServiceTasksFromStorageAndReloadTableView];
    }
...
// Added by Harshitha to call ServiceTask api everytime app comes to foreground
    ...
    self.navigationController.navigationBar.barTintColor = [UIColor colorWithRed:255.0/255 green:143.0/255 blue:30.0/255 alpha:1.0];
...
//        if (!isOnlyRefresh)
        if (!isOnlyRefresh && appDelegateObj.callContextDataApiFlag)
        {
            isOnlyRefresh = NO;
...
- (void) getServiceTasksFromStorageAndReloadTableView
{
    ...
    GSPAppDelegate *appDelegateObj = (GSPAppDelegate *) [[UIApplication sharedApplication] delegate];
    if (self.screenType == serviceTaskOverView && appDelegateObj.isTaskForTodayScreen)
    {
...
//        [self createUnderLinedButtonsForHeader];
        ...
        [self.serviceTaskTabelView setFrame:CGRectMake(0, 0, 950, self.serviceTaskTabelView.frame.size.height)];
...
// Added by Harshitha
    GSPAppDelegate *appDelegateObj = (GSPAppDelegate *) [[UIApplication sharedApplication] delegate];
    appDelegateObj.activeApp = @"ServiceOrderEdit";
...
//        taskDetailVC = [[GSPServiceTaskDetailViewController alloc]initWithNibName:@"GSPServiceTaskDetailViewController_iPad" bundle:nil withObject:serviceTask atIndex:(int)indexPath.row andOrdersArray:serviceOrdersArray];
        ...
        if(UIInterfaceOrientationIsPortrait(interfaceOrientation))
            taskDetailVC = [[GSPServiceTaskDetailViewController alloc]initWithNibName:@"GSPServiceTaskDetailVC_iPad" bundle:nil withObject:serviceTask atIndex:(int)indexPath.row andOrdersArray:serviceOrdersArray];
...
// Added by Harshitha
    GSPAppDelegate *appDelegateObj = (GSPAppDelegate *) [[UIApplication sharedApplication] delegate];
    appDelegateObj.activeApp = @"ColleagueList";
